...
paths_to_costs_elephant = defaultdict(int)
paths_to_costs_elephant[tuple(['AA'])] = 0
max_found = 0
checked = set()
...
for i, option in enumerate(best_paths):
	...
	while q:
		...
		res = compute_value(curr_node, cost_so_far, path[:], set(elephant_valves).difference(path), paths_to_costs_elephant, True, sup)
